...
document.querySelectorAll("#email").forEach(function(el, idx) {
  ...
  document.addEventListener("mousemove", function(event) {
    let ghostBody = document.querySelector("#ghost-body");
    let ghostRect = ghostBody.getBoundingClientRect();
    let centerX = ghostRect.left + ghostRect.width / 2;
    let centerY = ghostRect.top + ghostRect.height / 2;

    let mouseX = event.clientX;
    let mouseY = event.clientY;

    let angleLeft = Math.atan2(mouseY - centerY, mouseX - (centerX - 36));
    let angleRight = Math.atan2(mouseY - centerY, mouseX - (centerX + 36));

    let pupilLeft = document.querySelector("#pupil-left");
    let pupilRight = document.querySelector("#pupil-right");

    let radius = 5; // raio de movimento das pupilas

    pupilLeft.setAttribute("cx", 120 + radius * Math.cos(angleLeft));
    pupilLeft.setAttribute("cy", 71 + radius * Math.sin(angleLeft));
    pupilRight.setAttribute("cx", 84 + radius * Math.cos(angleRight));
    pupilRight.setAttribute("cy", 69 + radius * Math.sin(angleRight));
});
